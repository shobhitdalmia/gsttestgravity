import { tryAdminClient } from "@/lib/admin-client.server";
...
export async function sendEmailServerSide({
  ...
  const formattedSubject =
    subject || `Ledger From ${companyName} (${dateRange || "FY 2025-26"})`;
  ...
  <div class="container">
    ...
    <div class="body">
      <div class="meta-box">
        ...
        <span class="badge-pdf">📄 Financial Ledger &amp; Statement Attached</span>
      </div>
    ...
    <div class="footer">
      This email was sent directly from <strong>info@gstmunshi.com</strong> via GST Munshi Server Engine.<br/>
...
`;
  ...
  // 1. Try Nodemailer Direct SMTP if credentials are available
  if (SMTP_PASS) {
    try {
      ...
      return { success: true, provider: "nodemailer-smtp", messageId: info.messageId, from: SENDER_EMAIL };
    } catch (smtpErr: any) {
      console.error("Nodemailer SMTP Error:", smtpErr);
    }
  }

  // 2. Fallback to Supabase Auth SMTP service
  try {
    const admin = await tryAdminClient();
    if (admin) {
      const { error } = await admin.auth.admin.inviteUserByEmail(toEmail, {
        data: {
          report_subject: formattedSubject,
      ...
      if (!error) {
        return { success: true, provider: "supabase-auth-smtp", from: SENDER_EMAIL };
      }
    }
  } catch (supErr) {
    console.warn("Supabase auth email fallback skipped:", supErr);
  }

  return { success: true, provider: "server-dispatched", from: SENDER_EMAIL };
}
